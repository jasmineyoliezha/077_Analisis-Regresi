...
```{r}
library(readxl)
dt <- read_excel("Player Goals AFCON 2025.xlsx")
dt
```
...
Berdasarkan perhitungan, didapat dugaan persamaan regresi linear sebagai berikut. Dugaan persamaan tersebut dapat pula disebut sebagai nilai harapan dari peubah respons Y (Goals per 90)

$$E[\hat{Y}] = \hat{Y} = 0.4483760 - 0.0001188 X$$
...
Oleh karena simpangan baku merupakan akar kuadrat dari ragam, maka nilai dugaan galat baku model yang kita bentuk adalah $S_e = \sqrt{0.03435416} \approx 0.1853488$
...
Sebelumnya, melalui fungsi `lm` dan perhitungan manual yang sudah dibahas pada pertemuan sebelumnya, kita sudah mendapatkan nilai dugaan parameter β0 dan β1 sebesar 0.448376 dan -0.0001188112 Kedua nilai dugaan parameter tersebut dapat kita definisikan sebagai berikut.
...
Berdasarkan output sintaks di atas, diketahui bahwa dugaan nilai individu $Y$ (Goals) ketika nilai $x$ (Minutes) = 10 adalah 79.10309. Selain itu, diindikasikan juga bahwa dalam taraf kepercayaan 95%, diyakini bahwa nilai amatan individu $Y$ ketika nilai $x = 10$ berada dalam selang 75.74925 hingga 82.45694.